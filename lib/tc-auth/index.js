...
exports.auth = function(app) {
  ...
  if (authEnabled) {
    ...
    checkPaths.addAuth(app, [jwtCheck.jwtCheck(config.get('auth0')), tcUser.tcUser, safeList.safeList, routeHelper.requireAuth]);
  } else {
    // fake auth
    app.use(tcUser.mockUser);
  }

  app.use(checkPaths.checkPath);
};
