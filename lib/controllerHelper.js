 ...
 * @param lookupModel the model to lookup when filtering by reference models
...
function _findEntityByFilter(model, lookupModel, filters, req, callback) {
  ...
  if (_.isNaN(idValue) || idValue < 0) {
    ...
    if (lookupModel) {
      var query = {};
      lookupModel.primaryKeyAttributes.forEach(function (primaryKey) {
        query[primaryKey] = req.swagger.params[primaryKey].value;
        if (primaryKey in filters.where) {
          delete filters.where[primaryKey];
        }
      });
      // validate the entity belongs to the associated lookupModel
      lookupModel.find(query).success(function() {
        findById(idValue);
      })
      .error(function(err) {
        routeHelper.addError(req, err);
...
function getAllEntities(model, referenceModels, lookupModel, options, req, res, next) {
  async.waterfall([
    ...
    function(filters, callback) {
      ...
      if (lookupModel) {
        var referenceIdField = routeHelper.getRefIdField(referenceModels[0]);
        if (referenceIdField in filters.where) {
          filters.where[lookupModel.tableName + '.' + referenceIdField] = filters.where[referenceIdField];
          delete filters.where[referenceIdField];
        }
        filters.include = [ lookupModel ];
      }
      model.findAndCountAll(filters).success(function(result) {
        if (lookupModel) {
          var name = lookupModel.name + 's';
          var keyToDelete = name.charAt(0).toLowerCase() + name.slice(1);
          result.rows.forEach(function (row) {
            if (keyToDelete in row) {
              delete row[keyToDelete];
            }
            ['dataValues', '_previousDataValues'].forEach(function (rowKey) {
              if (keyToDelete in row[rowKey]) {
                delete row[rowKey][keyToDelete];
              }
            });
          });
        }
 ...
 * @param next the next function in the chain
 */
function getEntity(model, referenceModels, lookupModel, options, req, res, next) {
  async.waterfall([
    function(callback) {
      _checkExtraParameters(req, callback);
    },
    ...
    function(filters, callback) {
      ...
      _findEntityByFilter(model, lookupModel, filters, req, callback);
    }
 ...
 * @param next the next function in the chain
 */
function createEntity(model, referenceModels, lookupModel, options, req, res, next) {
  async.waterfall([
    function(callback) {
      _checkExtraParameters(req, callback);
    },
    ...
    function(filters, callback) {
      ...
      model.create(data).success(function(entity) {
        if (lookupModel) {
          var lookupData = {};
          _.extend(lookupData, filters.where);
          lookupData[routeHelper.getRefIdField(model)] = entity.id;
          lookupModel.create(lookupData).success(function() {
 ...
 * @param next the next function in the chain
 */
function updateEntity(model, referenceModels, lookupModel, options, req, res, next) {
  async.waterfall([
    function(callback) {
      _checkExtraParameters(req, callback);
    },
 ...
 * @param next the next function in the chain
 */
function deleteEntity(model, referenceModels, lookupModel, options, req, res, next) {
  async.waterfall([
    function(callback) {
      _checkExtraParameters(req, callback);
    },
 ...
 */
exports.buildController = function(model, referenceModels, lookupModel, options) {
  var controller = {};

  // Get an entity.
  controller.get = function(req, res, next) {
    getEntity(model, referenceModels, lookupModel, options, req, res, next);
  };

  // Create an entity.
  controller.create = function(req, res, next) {
    createEntity(model, referenceModels, lookupModel, options, req, res, next);
  };

  // Update an entity.
  controller.update = function(req, res, next) {
    updateEntity(model, referenceModels, lookupModel, options, req, res, next);
  };

  // Retrieve all entities.
  controller.all = function(req, res, next) {
    getAllEntities(model, referenceModels, lookupModel, options, req, res, next);
  };

  // Delete an entity.
  controller.delete = function(req, res, next) {
    deleteEntity(model, referenceModels, lookupModel, options, req, res, next);
  };
