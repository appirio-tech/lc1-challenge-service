...
function getAllEntities(model, referenceModels, options, req, res, next) {
  async.waterfall([
    ...
    function(filters, callback) {
      ...
      if (filters.where && referenceModels) {
        var referenceIds = [];
        referenceModels.forEach(function (referenceModel) {
          referenceIds.push(routeHelper.getRefIdField(referenceModel));
        });
        _.keys(filters.where).forEach(function (filterKey) {
          if (referenceIds.indexOf(filterKey) !== -1) {
            filters.where[_toUnderscored(filterKey)] = filters.where[filterKey];
            delete filters.where[filterKey];
          }
        });
      }
